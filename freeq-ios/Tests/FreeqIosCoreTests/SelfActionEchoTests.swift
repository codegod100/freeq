...
///     hold the `echo-message` cap, so the author's view updates on echo.
...
/// view immediately?" — the same rule the macOS client uses. Ported so both
/// platforms share one audited definition.
