...
use datastar::prelude::{ElementPatchMode, ExecuteScript, PatchSignals};
        ...
        loop {
            match lines_rx.recv().await {
                Ok(line) => {
                    ...
                    // Emit a DataStar PatchElements event that appends a
                    // <script data-effect> to <body>. DataStar v1.0.2 runs
                    // these scripts, which lets us call a client helper that
                    // updates the chip count/title for reactions from other
                    // users. The current user's own .mine highlight is also
                    // refreshed via the helper.
                    if let Some((msgid, emoji, nick, added)) = parse_tagmsg_reaction(&line) {
                        if should_emit(&line, &channel) {
                            let safe_mid = html_escape(&msgid);
                            let safe_emoji = html_escape(&emoji);
                            let safe_nick = html_escape(&nick);
                            let script = format!(
                                r#"<script data-effect>window.updateReactionChipFromServer('{}','{}','{}',{})</script>"#,
                                safe_mid.replace('\'', "\\'"),
                                safe_emoji.replace('\'', "\\'"),
                                safe_nick.replace('\'', "\\'"),
...
fn render_reaction_chips(
    ...
    if let Some(mid) = msgid {
        chips.push_str(&format!(
            r#"<button class="react-btn" type="button" data-on:click="$picker_msgid='{mid}';$picker_open=true" title="React">+</button>"#
        ));
    }
    chips.push_str("</span>");
    chips
...
fn parse_tagmsg_reaction(line: &str) -> Option<(String, String, String, bool)> {
