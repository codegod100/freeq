...
/// Shared once so Topcoat does not register the same asset twice.
pub const EVENTS_JS: topcoat::asset::Asset = asset!("../static/events.js");
        ...
        if let Some(store) = &self.session_store {
            match store.load(session_id) {
                Ok(Some(oauth)) => {
                    ...
                    let nick = crate::irc_render::sanitize_nick(&oauth.handle);
