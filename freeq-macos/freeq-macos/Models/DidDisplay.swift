...
/// sender never sees their own message. Pure + unit-tested.
enum DmEcho {
    /// The (nick, did) binding to adopt from a self-echoed DM, or nil when
    /// there's nothing to bind: not our echo, a channel, no/again-a-DID key,
    /// or target already equals the DID.
